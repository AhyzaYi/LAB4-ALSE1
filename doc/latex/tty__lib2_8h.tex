\hypertarget{tty__lib2_8h}{}\section{include/tty\+\_\+lib2.h File Reference}
\label{tty__lib2_8h}\index{include/tty\+\_\+lib2.\+h@{include/tty\+\_\+lib2.\+h}}
...
\item 
void \hyperlink{tty__lib2_8h_af4b26b32c3c509fa307419652482c659}{config\+\_\+tty} (const char $\ast$tty\+\_\+port, struct termios $\ast$tty, unsigned int baud, int $\ast$serial\+\_\+port)
\end{DoxyCompactItemize}


\subsection{Function Documentation}
\mbox{\Hypertarget{tty__lib2_8h_af4b26b32c3c509fa307419652482c659}\label{tty__lib2_8h_af4b26b32c3c509fa307419652482c659}} 
\index{tty\+\_\+lib2.\+h@{tty\+\_\+lib2.\+h}!config\+\_\+tty@{config\+\_\+tty}}
\index{config\+\_\+tty@{config\+\_\+tty}!tty\+\_\+lib2.\+h@{tty\+\_\+lib2.\+h}}
...
98 \textcolor{comment}{        // ...}
99 \textcolor{comment}{        // ...}
100 \textcolor{comment}{        // Set custom baud rate}
101 \textcolor{comment}{        tty->c\_cflag &= ~CBAUD;}
102 \textcolor{comment}{        tty->c\_cflag |= CBAUDEX;}
103 \textcolor{comment}{        // On the internet there is also talk of using the "BOTHER" macro here:}
104 \textcolor{comment}{        // tty->c\_cflag |= BOTHER;}
105 \textcolor{comment}{        // I never had any luck with it, so omitting in favour of using}
106 \textcolor{comment}{        // CBAUDEX}
107 \textcolor{comment}{        tty->c\_ispeed = 123456; // What a custom baud rate!}
108 \textcolor{comment}{        tty->c\_ospeed = 123456;}
109 \textcolor{comment}{        // Write terminal settings to file descriptor}
110 \textcolor{comment}{        ioctl(*serial\_port, TCSETS2, tty);}
111 \textcolor{comment}{    */}
112     
113     \textcolor{comment}{// Save tty settings, also checking for error}
114     \textcolor{keywordflow}{if} (tcsetattr(*serial\_port, TCSANOW, tty) != 0) \{
115         printf(\textcolor{stringliteral}{"Error %i from tcsetattr: %s\(\backslash\)n"}, errno, strerror(errno));
116     \}
117     \textcolor{comment}{/*********/}
118     \textcolor{comment}{/*WRITING*/}
119     \textcolor{comment}{/*********/}
120     \textcolor{comment}{//unsigned char msg[] = \{ 'H', 'e', 'l', 'l', 'o', '\(\backslash\)r' \};}
121     \textcolor{comment}{//write(*serial\_port, msg, sizeof(msg));}
122 
123     \textcolor{comment}{/*********/}
124     \textcolor{comment}{/*READING*/}
125     \textcolor{comment}{/*********/}
126       \textcolor{comment}{// Allocate memory for read buffer, set size according to your needs}
127     \textcolor{comment}{//char read\_buf [256];}
128     
129     \textcolor{comment}{// Normally you wouldn't do this memset() call, but since we will just receive}
130     \textcolor{comment}{// ASCII data for this example, we'll set everything to 0 so we can}
131     \textcolor{comment}{// call printf() easily.}
132     \textcolor{comment}{//memset(&read\_buf, '\(\backslash\)0', sizeof(read\_buf));}
133     
134     \textcolor{comment}{// Read bytes. The behaviour of read() (e.g. does it block?,}
135     \textcolor{comment}{// how long does it block for?) depends on the configuration}
136     \textcolor{comment}{// settings above, specifically VMIN and VTIME}
137     \textcolor{comment}{//int num\_bytes = read(*serial\_port, &read\_buf, sizeof(read\_buf));}
138     
139     \textcolor{comment}{// n is the number of bytes read. n may be 0 if no bytes were received, and can also be -1 to signal an
       error.}
140     \textcolor{comment}{//if (num\_bytes < 0) \{}
141     \textcolor{comment}{//  printf("Error reading: %s", strerror(errno));}
142     \textcolor{comment}{//  return 1;}
143     \textcolor{comment}{//\}}
144     
145     \textcolor{comment}{// Here we assume we received ASCII data, but you might be sending raw bytes (in that case, don't try
       and}
146     \textcolor{comment}{// print it to the screen like this!)}
147     \textcolor{comment}{//printf("Read %i bytes. Received message: %s", num\_bytes, read\_buf);}
148 
149     \textcolor{comment}{//close(serial\_port);}
150 
151 \}
\end{DoxyCode}
